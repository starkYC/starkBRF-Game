...
static MJRefreshHeaderView *header = nil;
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state) {
        ...
        {
            // 设置文字
            _statusLabel.text = NSLocalizedString(MJRefreshHeaderReleaseToRefresh, nil);
            // 执行动画
        ...
        {
            // 设置文字
			_statusLabel.text = NSLocalizedString(MJRefreshHeaderPullToRefresh, nil);
            // 执行动画
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
        ...
        {
            // 设置文字
            _statusLabel.text = NSLocalizedString(MJRefreshHeaderRefreshing, nil);
            // 执行动画
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                _arrowImage.transform = CGAffineTransformIdentity;
                // 1.增加65的滚动区域
